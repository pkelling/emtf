...
(
	...
	//output [15:0] refclk,
	//output [15:0] refclk_odiv
	output clk125_nob
);
    wire [15:0] refclk; // Had to add
	...
	assign_mgt_rx ari30 (mgt_rx_if [9][2], irpc_rx [0]);
	assign_mgt_tx ati31 (mgt_tx_if [10][0], daq_tx [0]);
	assign_mgt_rx ari32 (mgt_rx_if [10][2], ge11_rx [5]);
	assign_mgt_rx ari33 (mgt_rx_if [10][3], ge11_rx [4]);
	assign_mgt_rx ari34 (mgt_rx_if [11][0], ge11_rx [1]);
	assign_mgt_rx ari35 (mgt_rx_if [11][1], ge11_rx [0]);
	assign_mgt_rx ari36 (mgt_rx_if [11][2], mpcn_rx [8]);
	assign_mgt_rx ari37 (mgt_rx_if [12][0], mpcn_rx [4]);
	assign_mgt_rx ari38 (mgt_rx_if [12][1], mpcn_rx [5]);
	assign_mgt_rx ari39 (mgt_rx_if [12][2], mpcn_rx [0]);
	assign_mgt_rx ari40 (mgt_rx_if [12][3], mpcn_rx [1]);
	assign_mgt_rx ari41 (mgt_rx_if [13][0], cppf_rx [3]);
	assign_mgt_rx ari42 (mgt_rx_if [13][1], cppf_rx [4]);
	assign_mgt_rx ari43 (mgt_rx_if [13][2], cppf_rx [5]);
	assign_mgt_rx ari44 (mgt_rx_if [13][3], cppf_rx [6]);
	assign_mgt_rx ari45 (mgt_rx_if [14][0], mpc4_rx [5]);
	assign_mgt_rx ari46 (mgt_rx_if [14][1], mpc4_rx [4]);
	assign_mgt_rx ari47 (mgt_rx_if [14][2], mpc4_rx [1]);
	assign_mgt_rx ari48 (mgt_rx_if [14][3], mpc4_rx [0]);
	assign_mgt_rx ari49 (mgt_rx_if [15][0], mpc3_rx [5]);
	assign_mgt_rx ari50 (mgt_rx_if [15][1], mpc3_rx [4]);
	assign_mgt_rx ari51 (mgt_rx_if [15][2], mpc3_rx [1]);
	assign_mgt_rx ari52 (mgt_rx_if [15][3], mpc3_rx [0]);
	assign_mgt_rx ari53 (mgt_rx_if [16][0], mpc2_rx [5]);
	assign_mgt_rx ari54 (mgt_rx_if [16][1], mpc2_rx [4]);
	assign_mgt_rx ari55 (mgt_rx_if [16][2], mpc2_rx [1]);
	assign_mgt_rx ari56 (mgt_rx_if [16][3], mpc2_rx [0]);
	assign_mgt_rx ari57 (mgt_rx_if [17][0], mpc1_rx [5]);
	assign_mgt_rx ari58 (mgt_rx_if [17][1], mpc1_rx [4]);
	assign_mgt_rx ari59 (mgt_rx_if [17][2], mpc1_rx [1]);
	assign_mgt_rx ari60 (mgt_rx_if [17][3], mpc1_rx [0]);
	assign_mgt_rx ari61 (mgt_rx_if [18][0], mpc0_rx [5]);
	assign_mgt_tx ati61 (mgt_tx_if [18][0], gmt_tx [1]);
	assign_mgt_rx ari62 (mgt_rx_if [18][1], mpc0_rx [1]);
	assign_mgt_rx ari63 (mgt_rx_if [18][2], mpc0_rx [4]);
	assign_mgt_rx ari64 (mgt_rx_if [18][3], mpc0_rx [0]);
	...
	assign com_gtrefclk0 [9] = refclk [0]; // irpc_rx 0
	assign mgt_gtrefclk0 [10][0] = refclk [0]; // daq_tx 0
	assign com_gtrefclk0 [10] = refclk [8]; // ge11_rx 5
	assign com_gtrefclk0 [10] = refclk [8]; // ge11_rx 4
	assign com_gtrefclk0 [11] = refclk [9]; // ge11_rx 1
	assign com_gtrefclk0 [11] = refclk [9]; // ge11_rx 0
	assign mgt_gtrefclk0 [11][2] = refclk [9]; // mpcn_rx 8
	assign mgt_gtrefclk0 [12][0] = refclk [9]; // mpcn_rx 4
	assign mgt_gtrefclk0 [12][1] = refclk [9]; // mpcn_rx 5
	assign mgt_gtrefclk0 [12][2] = refclk [9]; // mpcn_rx 0
	assign mgt_gtrefclk0 [12][3] = refclk [9]; // mpcn_rx 1
	assign com_gtrefclk0 [13] = refclk [1]; // cppf_rx 3
	assign com_gtrefclk0 [13] = refclk [1]; // cppf_rx 4
	assign com_gtrefclk0 [13] = refclk [1]; // cppf_rx 5
	assign com_gtrefclk0 [13] = refclk [1]; // cppf_rx 6
	assign mgt_gtrefclk0 [14][0] = refclk [10]; // mpc4_rx 5
	assign mgt_gtrefclk0 [14][1] = refclk [10]; // mpc4_rx 4
	assign mgt_gtrefclk0 [14][2] = refclk [10]; // mpc4_rx 1
	assign mgt_gtrefclk0 [14][3] = refclk [10]; // mpc4_rx 0
	assign mgt_gtrefclk0 [15][0] = refclk [10]; // mpc3_rx 5
	assign mgt_gtrefclk0 [15][1] = refclk [10]; // mpc3_rx 4
	assign mgt_gtrefclk0 [15][2] = refclk [10]; // mpc3_rx 1
	assign mgt_gtrefclk0 [15][3] = refclk [10]; // mpc3_rx 0
	assign mgt_gtrefclk0 [16][0] = refclk [11]; // mpc2_rx 5
	assign mgt_gtrefclk0 [16][1] = refclk [11]; // mpc2_rx 4
	assign mgt_gtrefclk0 [16][2] = refclk [11]; // mpc2_rx 1
	assign mgt_gtrefclk0 [16][3] = refclk [11]; // mpc2_rx 0
	assign mgt_gtrefclk0 [17][0] = refclk [11]; // mpc1_rx 5
	assign mgt_gtrefclk0 [17][1] = refclk [11]; // mpc1_rx 4
	assign mgt_gtrefclk0 [17][2] = refclk [11]; // mpc1_rx 1
	assign mgt_gtrefclk0 [17][3] = refclk [11]; // mpc1_rx 0
	assign com_gtrefclk0 [18] = refclk [3]; // gmt_tx 1
	assign mgt_gtrefclk0 [18][0] = refclk [11]; // mpc0_rx 5
	assign mgt_gtrefclk0 [18][1] = refclk [11]; // mpc0_rx 1
	assign mgt_gtrefclk0 [18][2] = refclk [11]; // mpc0_rx 4
	assign mgt_gtrefclk0 [18][3] = refclk [11]; // mpc0_rx 0
	BUFG CPPF_3_inst (.O(CPPF_3_mmcm_clk), .I(mgt_tx_if[18][0].txoutclk));
	assign mgt_txusrclk[18][0] = CPPF_3_mmcm_clk; // gmt_tx1
	BUFG DAQ_0_inst (.O(DAQ_0_mmcm_clk), .I(mgt_tx_if[10][0].txoutclk));
	assign mgt_txusrclk[10][0] = DAQ_0_mmcm_clk; // daq_tx0
