...
(
	...
	input [7:0] crc_err_flag     [4:0], // CRC error detected [station][link]
	...
	begin
		...
		begin
			case (reg_addr)
                ...
                9'h05c: begin jtag_length     = r_in; end
                9'h05d: begin jtag_tms_vector = r_in; end
                9'h05e: begin jtag_tdi_vector = r_in; end
		...
		begin
            // jtag register logic
            if (jtag_done == 1'b1) // when done serializing 
            begin
                jtag_enable = 1'h0; // remove enable flag
                jtag_in3_r = jtag_tdo_vector; // load tdo vector
            end
        end
	end
    ...
    wire [48:0] err_tst_pat_flag_comb = 
    {
        err_tst_pat_flag_n,
        err_tst_pat_flag[4],
        err_tst_pat_flag[3],
    ...
    wire [63:0] hmt_rate_comb = {hmt_rate[2], hmt_rate[1]};
...
//				9'h026: begin r_out = r_out | ddr_clk_del; end
                ...
                9'h05c: begin r_out = r_out | jtag_length; end
                9'h05d: begin r_out = r_out | jtag_tms_vector; end
                9'h05e: begin r_out = r_out | jtag_tdi_vector; end
                9'h05f: begin r_out = r_out | jtag_in3_r; end
