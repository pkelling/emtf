...
module PIO_TX_ENGINE    #(
  ...
  localparam PIO_TX_RST_STATE  = 6'b000000;
  localparam PIO_TX_CPLD_QW1   = 6'b000001;
  ...
  reg [31:0]         tmp;
     ...
     */
  /*
     always @ (rd_be or req_addr or req_compl_wd_q) begin

     casex ({req_compl_wd_q, rd_be[3:0]})
   ...
   *  and Bus Mastering 
    ...
    always @ ( posedge clk ) begin
        ...
        if (!rst_n ) begin
			...
			if (req_compl) // need to remember completion request for delayed completion after beaming
				req_compl_q <=  1'b1;
			...
			case ( state )
				...
				PIO_TX_RST_STATE : begin
					...
					begin
						beam_addr = beam_addr; // system memory address for transfer
