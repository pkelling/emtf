...
int write_mrs(int fd, uint32_t cs, uint32_t code);
...
{
//    int fd = device_d;
  uint32_t value; //, rbval;
    // reset link
    value = 0;
    mwrite (fd, &value, 4, REG_BASE+0x14);
    usleep(10000);
    value = 1;
    mwrite (fd, &value, 4, REG_BASE+0x14);
    usleep(10000);
    ...
    // printf ("test control register\n");
    // for (int i  = 0; i < 10000; i++)
    //   {
    // 	value = (1 << (i%32));
    // 	mwrite (fd, &value, 4, REG_BASE+0xc);
    // 	mread  (fd, &rbval, 4, REG_BASE+0xc);
    // 	if (rbval != value) 
    // 	  printf ("mismatch: i: %03d value: %08x rb: %08x\n", i, value, rbval);
    // 	if (i < 32) printf ("rb: %08x\n", rbval);
    //   }
...
uint64_t *ptlut_data_buf = NULL;
uint32_t *ptlut_addr_buf = NULL;
...
int read_ptlut_file()
{
  ...
  ptlut_data_buf = (uint64_t*) malloc(RL_DATA_SIZE_B);
  if (ptlut_data_buf == NULL) {log_printf("data_buf: not enough memory\n"); return -1;}

  ptlut_addr_buf = (uint32_t*) malloc(RL_ADDR_SIZE_B);
  if (ptlut_addr_buf == NULL) {log_printf("addr_buf: not enough memory\n"); return -1;}
  

  for (uint64_t i = 0; i < RL_ADDR_SIZE_B/4; i++)
    {
      // just fill addresses for file contents
      ptlut_addr_buf[i] = (i*2); // address progresses by 2 because two words at a time are written
    }
  ...
  if (ptlut_in != NULL)
    {
      ...
      size_t result = fread (ptlut_data_buf, 1, RL_DATA_SIZE_B, ptlut_in);
      if (result != RL_DATA_SIZE_B) {log_printf ("reading failure, read bytes: 0x%llx\n", result);}
      fclose (ptlut_in);
    }
  else 
    {
      printf ("cannot open file: /opt/madorsky/data/ptlut.dat\n");
      return -1;
    }
...
int write_rldram(int th_dev)
{
    ...
    int fd = th_dev;
    uint64_t value;
    uint64_t *data_buf = ptlut_data_buf;
    uint32_t *addr_buf = ptlut_addr_buf;
	/*
	struct timespec t_res;
	int res;
	
	res = clock_getres(CLOCK_REALTIME, &t_res);
	printf ("CLOCK_REALTIME: %d s %d ns %d res\n", t_res.tv_sec, t_res.tv_nsec, res);

	res = clock_getres(CLOCK_REALTIME_COARSE, &t_res);
	printf ("CLOCK_REALTIME_COARSE: %d s %d ns %d res\n", t_res.tv_sec, t_res.tv_nsec, res);

	res = clock_getres(CLOCK_MONOTONIC, &t_res);
	printf ("CLOCK_MONOTONIC: %d s %d ns %d res\n", t_res.tv_sec, t_res.tv_nsec, res);

	res = clock_getres(CLOCK_MONOTONIC_COARSE, &t_res);
	printf ("CLOCK_MONOTONIC_COARSE: %d s %d ns %d res\n", t_res.tv_sec, t_res.tv_nsec, res);

	res = clock_getres(CLOCK_MONOTONIC_RAW, &t_res);
	printf ("CLOCK_MONOTONIC_RAW: %d s %d ns %d res\n", t_res.tv_sec, t_res.tv_nsec, res);

	res = clock_getres(CLOCK_PROCESS_CPUTIME_ID, &t_res);
	printf ("CLOCK_PROCESS_CPUTIME_ID: %d s %d ns %d res\n", t_res.tv_sec, t_res.tv_nsec, res);

	res = clock_getres(CLOCK_THREAD_CPUTIME_ID, &t_res);
	printf ("CLOCK_THREAD_CPUTIME_ID: %d s %d ns %d res\n", t_res.tv_sec, t_res.tv_nsec, res);
	*/
    // module addresses
...
//        memset (addr_buf, 0, RL_ADDR_SIZE_B);
    ...
    write_mrs(fd, 0x01010101, ODT_ON); // turn ODT on, only on one chip at the end of each quad
...
*/
    ...
    for (int i = 0; i < RL_BUFS; i++)
    {
	  ...
	  do
        {
		  // wait until not busy
		  mread (fd, &value, 8, saddr_stat);
		  //mwrite(fd, &value, 8, saddr_stat);

        } while ((value >> 21) & 1ULL);
	
	  //clock_gettime(CLOCK_REALTIME, &t_end);

	  //printf ("PT LUT poll time: %d ns, poll: %d\n", t_end.tv_nsec - t_start.tv_nsec, poll_cnt);

    }

    write_mrs(fd, 0xffffffff, ODT_OFF); // turn ODT off
...
int read_rldram_random_addr(int iterations)
{
    ...
    uint32_t *rand_addr;
    uint64_t *rb_data;
    //uint32_t older_rand = 0; 
	uint32_t old_rand = 0, new_rand;
    uint32_t MEM_BASE_PT_D = 0x140000; // pt data buffer
    uint32_t MEM_BASE_PT_A = 0x142000; // pt address buffer
    int ch = REG_BANK_CH; // config register bank
...
//                if (older_bank == new_bank && older_chip == new_chip) goto get_another;
        ...
        memset(rb_data, 0x55, FW_DATA_SIZE_B);
        // read FW buffer
        for (int j = 0; j < XFERS_FW_DATA; j++)
            mread(fd,
    ...
    free (rb_data);
    free (data_buf);
    if (err_count > 0) log_printf("ERRORS: %d\n", err_count);
    else log_printf ("done\n");
    return 0;
}
...
//    write_mrs(fd, 0x01010101, 0x4000c); // 120 Ohm
...
//    write_mrs(fd, 0xffffffff, ODT_OFF);
...
//        write_mrs(fd, cs_mask, value);
    ...
    uint64_t core_req_mask = 7ULL; // ptlut requests enable mask
