...
void write_verify_ge11_luts(bool verify, int endcap[13], int sector[13])
{
    int mismatch_cnt;
    ...
    for (int id = 0; id < 13; id++)
    {
        if (devices_d[id] >= 0)
        {
            ...
            // file name templates
            std::string th_lut_fname  = "ge11_lut/th_mem_endcap_E_sect_S.lut";
            std::string ph_init_fname = "ge11_lut/ph_init_endcap_E_sect_S.lut";
            ...
            th_lut_fname.replace(th_lut_fname.find("E"), 1, endcap_string);
            th_lut_fname.replace(th_lut_fname.find("S"), 1, sector_string);

            ph_init_fname.replace(ph_init_fname.find("E"), 1, endcap_string);
            ph_init_fname.replace(ph_init_fname.find("S"), 1, sector_string);
            ...
            uint32_t ch_th = 56; // cs[6][2] in firmware, 6*9+2 = 56
            uint32_t ch_ph = 55; // cs[6][1] in firmware, 6*9+1 = 55


            // **********************************************************
            // theta LUTs

            uint64_t th_lut_data [14];
            uint64_t th_lut_rb   [14];
            uint64_t th_lut_val;

            // read theta luts from file first
            FILE* thin = fopen (th_lut_fname.c_str(), "r");
            if (thin != NULL)
            {
                for (int i = 0; i < 14; i++) // 64-bit word loop
                {
                    th_lut_data[i] = 0ULL;
                    for (int j = 0; j < 8; j++) // byte loop inside 64-bit word
                    {
                        int rsp = fscanf (thin, "%llx", &th_lut_val);
                        if (rsp != 1) break;
                        th_lut_data[i] >>= 8; // move previous bytes to the right
                        th_lut_data[i] |= (th_lut_val << 56); // plug in new byte
                    }
                }
                fclose (thin);
            }
            ...
            if (!verify)
            {
                ...
                uint32_t saddr = MEM_BASE + (ch_th << 12); // th_lut, addr = 0 at start

                // write words into LUT. Size is in bytes
                mwrite(fd, th_lut_data, 14*8, saddr);
            }
            ...
            long long unsigned ph_init_data[14];
            long long unsigned ph_init_rb[14];

            // read phi init parameters from files
            thin = fopen (ph_init_fname.c_str(), "r");
            if (thin != NULL)
            {
                for (int i = 0; i < 14; i++)
                {
                    int rsp = fscanf (thin, "%llx", &(ph_init_data[i]));
                    if (rsp != 1) break;
                }
                fclose (thin);
            }
            ...
            {
                // form start address {chamber[6], sel[2], addr[7], 3'b0}
                uint32_t saddr = MEM_BASE + (ch_ph << 12); // addr = 0 at start

                // write words into LUT. Size is in bytes
                mwrite(fd, ph_init_data, 14*8, saddr);
            }
            ...
            // form start address {chamber[6], sel[2], addr[7], 3'b0}
            uint32_t saddr = MEM_BASE + (ch_th << 12); // addr = 0 at start

            // read words from LUT. Size is in bytes
            mread(fd, th_lut_rb, 14*8, saddr);

            // compare
            for (int i = 0; i < 14; i++)
            {
                uint64_t xorm = th_lut_data[i] ^ th_lut_rb[i];
                if (xorm != 0)
                {
                    log_printf ("i: %03d w: %016llx r: %016llx e: %016llx\n",
                                i, th_lut_data[i], th_lut_rb[i], xorm);
                    mismatch_cnt++;
                    if (mismatch_cnt > 200) goto eject;
                }
            }
            log_printf ("GE1/1 theta LUTs read and verified\n");

            // form start address {chamber[6], sel[2], addr[7], 3'b0}
            saddr = MEM_BASE + (ch_ph << 12); // addr = 0 at start

            // read words from LUT. Size is in bytes
            mread(fd, ph_init_rb, 14*8, saddr);

            // parameter comparison
            for (int i = 0; i < 14; i++) // chamber loop
            {
                uint64_t xorm = ph_init_data[i] ^ ph_init_rb[i];
                if (xorm != 0)
                {
                    log_printf ("ph_init ch: %02d w: %016llx r: %016llx e: %016llx\n",
            ...
            log_printf ("GE1/1 ph init parameters read and verified\n");
        }
    }
eject:
...
void write_verify_pc_luts(bool verify, int endcap[13], int sector[13], bool ge11_prim)
{
    if (ge11_prim)
    {
        write_verify_ge11_luts(verify, endcap, sector);
        return;
    }
