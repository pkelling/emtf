...
/* EMTF JTAG registers addresses 64-bits */
#define EMTF_JTAG_TIMEOUT 100000  // JTAG Timeout 

#define EMTF_JTAG_LEN  0xB62E0 //	8	00000000ffffffff	1	1	Debug hub JTAG length
#define EMTF_JTAG_TMS  0xB62E8 //	8	00000000ffffffff	1	1	Debug hub JTAG TMS vector
#define EMTF_JTAG_TDI  0xB62F0 //	8	00000000ffffffff	1	1	Debug hub JTAG TDI vector
#define EMTF_JTAG_TDO  0xB62F8 //	8	00000000ffffffff	1	1	Debug hub JTAG TDO vector
#define EMTF_JTAG_CTRL 0xB6300 //	8	0000000000000001	1	1	Debug hub JTAG enable/busy
...
#include <sys/ioctl.h>
#include "xvc_ioctl.h"
#define CHAR_DEV_PATH   "/dev/utca_sp120"
static char* char_dev = CHAR_DEV_PATH;
#endif /* !USE_IOCTL */
...
int handle_data(int fd, int fd_ioctl) {
	...
	uint64_t ctrl_reg = 0;
	    ...
	    pwrite(fd_ioctl, &shift_num_bits, 8, EMTF_JTAG_LEN);	 
   
            memcpy(&tms, &buffer[byteIndex], shift_num_bytes);
	    pwrite(fd_ioctl, &tms, 8, EMTF_JTAG_TMS);

            memcpy(&tdi, &buffer[byteIndex + nr_bytes], shift_num_bytes);
	    pwrite(fd_ioctl, &tdi, 8, EMTF_JTAG_TDI);

	    // Initiate JTAG bit shift
	    ctrl_reg = 0x1; 
            pwrite(fd_ioctl, &ctrl_reg, 8, EMTF_JTAG_CTRL);

            timeout_cnt = 0;
	    while ((ctrl_reg == 1) && (timeout_cnt<EMTF_JTAG_TIMEOUT)) {
	      pread(fd_ioctl, &ctrl_reg, 8, EMTF_JTAG_CTRL);
	      usleep(1);
	      timeout_cnt++;
	    }
            if (timeout_cnt>=EMTF_JTAG_TIMEOUT) {
		fprintf(stderr, "xvc ioctl error: EMFT JTAG timeout\n");
		return -1;
	    }
	    ...
	    */
            ...
            pread(fd_ioctl, &tdo, 8, EMTF_JTAG_TDO);
            memcpy(&result[byteIndex], &tdo, shift_num_bytes);
...
int main(int argc, char **argv) {
    ...
    fprintf(stdout, "Opened xvc ioctl device driver: %s\n", char_dev);
